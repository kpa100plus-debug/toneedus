...
const checks = [
  ...
  ['최고·부관리자 권한 분리', files.worker.includes('PRIMARY_ADMIN_EMAIL') && files.worker.includes('admin_roles') && files.worker.includes('requirePrimaryAdmin') && files.worker.includes('appointDeputy') && files.client.includes('appointDeputy') && files.app.includes('renderDeputyAdmin') && files.migration.includes("admin_roles") && files.wrangler.includes('kpa100plus@gmail.com')],
