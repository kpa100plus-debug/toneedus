...
const checks = [
  ...
  ['최고관리자 공지 푸시 발송', files.worker.includes('/api/admin/push-announcements') && files.worker.includes('sendAdminPushAnnouncement') && files.worker.includes('requirePrimaryAdmin(request, env)') && files.worker.includes('PUSH_CONFIRMATION_REQUIRED') && files.worker.includes('push_announcement_logs') && files.client.includes('sendPushAnnouncement') && files.app.includes('open-push-announcement') && files.app.includes('push-announcement-form')],
