...
try{
 ...
 let one=await send(a);
 const initial=(await req('/api/me/email-verification',undefined,a)).body.challenge;
 assert.ok(initial.resendAt-Date.now()>179000);assert.equal(initial.expiresAt-initial.resendAt,420000);
 sql.prepare('UPDATE email_otp_send_locks SET sent_at=? WHERE user_id=?').run(Date.now()-179000,a.id);
 assert.equal((await req('/api/me/email-verification/send',{},a)).body.error.code,'EMAIL_RESEND_WAIT');
 sql.prepare('UPDATE email_otp_send_locks SET sent_at=? WHERE user_id=?').run(Date.now()-181000,a.id);
 one=await send(a);
 pass('resend is blocked before 180 seconds and allowed after, code stays valid for ten minutes');assert.equal(mail.to[0].email,a.email);assert.match(one.code,/^\d{6}$/);
 ...
 const reapplied=await req(`/api/challenges/${id}/teasers`,teaser,b);assert.equal(reapplied.status,201);
 const shortlist=()=>req(`/api/challenges/${id}/shortlist`,{teaserId:reapplied.body.teaser.id},a);
 sql.prepare('UPDATE users SET email_verified=0 WHERE id=?').run(a.id);
 assert.equal((await shortlist()).body.error.code,'EMAIL_VERIFICATION_REQUIRED');
 sql.prepare('UPDATE users SET email_verified=1 WHERE id=?').run(a.id);
 sql.prepare('UPDATE users SET email_verified=0 WHERE id=?').run(b.id);
 assert.equal((await shortlist()).body.error.code,'CANDIDATE_EMAIL_REQUIRED');
 sql.prepare('UPDATE users SET email_verified=1 WHERE id=?').run(b.id);
 assert.equal((await shortlist()).status,200);
 assert.equal(sql.prepare('SELECT status FROM teasers WHERE id=?').get(reapplied.body.teaser.id).status,'SHORTLISTED');
 assert.equal((await req(`/api/challenges/${id}/shortlist`,{teaserId:reapplied.body.teaser.id,mode:'select'},a)).body.error.code,'VERIFICATION_REQUIRED');
 pass('both emails required for shortlist; email-only shortlist succeeds without unlocking final transactions');
