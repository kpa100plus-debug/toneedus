...
function oauthCompletionPage(request, { target, cookie = '' }) {
  ...
  return new Response(`<!doctype html><html lang="ko"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>모두의 챌린지</title></head><body><p>모두의 챌린지로 돌아가는 중입니다…</p><script>window.location.replace(${JSON.stringify(destination)});</script></body></html>`, {
    status: 200,
    headers: {
...
async function sendPushNotification(env, userId, { title, body, route = 'notifications', challengeId = null }) {
  ...
  const payload = JSON.stringify({
    title: cleanText(title, 1, 80),
    body: cleanText(body, 1, 140),
    ...
    url: `/#/${String(route || 'notifications').replace(/^\/+/, '')}`,
...
async function deliverWebPush(subscription, payload, env) {
  ...
  const ikm = await hkdf(new Uint8Array(sharedSecret), authSecret, concatBytes(encoder.encode('WebPush: info\0'), clientPublicKey), 32);
...
async function moderationQueue(request, env) {
  ...
  const result = await env.DB.prepare(`SELECT c.id, c.title, c.reward_amount, c.deadline, c.created_at, c.moderation_reasons_json,
    ...
    FROM challenges c WHERE c.status = 'REVIEW' ORDER BY c.created_at ASC LIMIT 30`).all();
...
async function addModerationNote(challengeId, request, env) {
  ...
  if (admin instanceof Response) return admin;
  const challenge = await fetchChallenge(challengeId, env);
  if (!challenge || challenge.status !== 'REVIEW') return problem(409, 'MODERATION_NOT_PENDING', '관리자 검토 대기 상태의 챌린지만 처리할 수 있습니다.');
  const body = await readJson(request);
  if (body instanceof Response) return body;
  ...
  if (requestApproval) {
    ...
    if (primary?.id) await sendPushNotification(env, primary.id, {
      ...
      body: '관리자 검토 대기 챌린지에 승인 요청이 등록되었습니다.',
      route: 'admin',
      challengeId,
    });
  }
...
async function submitTeaser(challengeId, request, env) {
  ...
  await sendPushNotification(env, challenge.owner_id, {
    ...
    body: '내 챌린지에 새 제안이 등록되었습니다. 앱에서 확인하세요.',
    route: 'challenge',
    challengeId,
  });
...
async function shortlistTeaser(challengeId, request, env) {
  ...
  await sendPushNotification(env, teaser.solver_id, {
    ...
    body: '내 제안이 후보 목록에 포함되었습니다. 앱에서 확인하세요.',
    route: 'challenge',
    challengeId,
  });
