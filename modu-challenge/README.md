...
- 공개 미션·TEASER·참여 신호·보상금·성공 상태를 현재 API 데이터에서 자동 집계합니다.
...
- 첫 화면에 필요한 설정·상태·공개 미션을 `/api/bootstrap` 한 번으로 불러옵니다.
- 회원 확인과 공개 데이터를 병렬로 요청해 초기 API 왕복 단계를 줄였습니다.
- 취소된 자동검수 미션은 기본 공개 목록에서 제외합니다.
- 버전이 지정된 정적 자산과 PWA 화면은 캐시 우선·백그라운드 갱신 방식으로 다시 방문할 때 빠르게 표시합니다.
- D1 공개 목록 정렬 경로에 전용 인덱스를 적용합니다.

## 확정 기준

- 공개 관리자·프로필 표시명: `SUPER ADMIN`
- 운영·권리 주체: ㈜ISEA GROUP
