...
const initialTheme=await req('/api/bootstrap');assert.equal(initialTheme.body.config.homeTheme,'original');
assert.equal((await req('/api/admin/home-theme',{theme:'emerald',revision:1},b.cookie)).status,403);
sql.prepare('UPDATE users SET is_admin=1 WHERE id IN (?,?)').run(a.body.user.id,b.body.user.id);
sql.prepare("INSERT OR REPLACE INTO admin_roles(user_id,role) VALUES (?,'primary')").run(a.body.user.id);
sql.prepare("INSERT OR REPLACE INTO admin_roles(user_id,role) VALUES (?,'deputy')").run(b.body.user.id);
const overviewTheme=await req('/api/admin/overview',undefined,a.cookie);assert.equal(overviewTheme.body.overview.homeTheme.theme,'original');
assert.equal((await req('/api/admin/home-theme',{theme:'emerald',revision:1},b.cookie)).status,403);
assert.equal((await req('/api/admin/home-theme',{theme:'<img>',revision:1},a.cookie)).status,400);
const themeChange=await req('/api/admin/home-theme',{theme:'emerald',revision:1},a.cookie);assert.equal(themeChange.status,200);assert.equal(themeChange.body.homeTheme.revision,2);
...
assert.equal((await req('/api/me/verifications')).status,401);const verificationList=await req('/api/me/verifications',undefined,b.cookie);assert.equal(verificationList.status,200);assert.equal(verificationList.body.enforcement,'advisory');assert.deepEqual(verificationList.body.requirementsBySubjectType.business,['IDENTITY','BUSINESS']);pass('member-only verification API separates subject requirements');
const providerRequired=await req('/api/me/verifications/requests',{type:'BUSINESS',subjectType:'business'},b.cookie);assert.equal(providerRequired.status,202);assert.equal(providerRequired.body.verification.status,'PROVIDER_REQUIRED');assert.equal(providerRequired.body.providerConnectionRequired,true);pass('unconnected provider never produces a false verified state');
...
const highCreate=await req('/api/challenges',{...challengeInput,title:'전국 공공시설 디자인 개선 제안',rewardAmount:3000000},b.cookie,'POST',env,{'Idempotency-Key':'reward-high-create-001'});assert.equal(highCreate.status,201);assert.equal(highCreate.body.moderationAction,'AUTO_APPROVED');assert.equal(highCreate.body.moderationRiskScore,25);assert.equal((await req('/api/challenges/'+highCreate.body.challenge.id)).status,200);pass('safe 3 million mission automatically publishes');
assert.equal((await req('/api/challenges/'+highCreate.body.challenge.id,undefined,(await req('/api/auth/login',{email:'a@test.invalid',passwordVerifier:material.passwordVerifier})).cookie,'GET')).body.context.canApply,true);pass('approved high reward mission remains open to applications despite an informational finding');
...
const replayChanges=await req('/api/challenges',{...challengeInput},b.cookie,'POST',env,{'Idempotency-Key':'moderation-changes-001'});assert.equal(replayChanges.status,200);assert.equal(replayChanges.body.duplicatePrevented,true);assert.equal(replayChanges.body.challenge.id,changesCreate.body.challenge.id);assert.equal(replayChanges.body.moderationAction,'CHANGES_REQUIRED');assert.equal(replayChanges.body.challenge.publicationVisibility,'private');assert.equal(replayChanges.body.challenge.visibility,'public');pass('repeated submission retains private moderation result and actual publication visibility');
const fixedChanges=await req('/api/challenges/'+changesCreate.body.challenge.id,{...challengeInput,title:'공개 자료 분류 업무 요청',description:'공개된 자료를 기준에 맞게 분류하고 출처와 결과표를 제출해주세요.'},b.cookie,'PUT');assert.equal(fixedChanges.status,200);assert.equal(fixedChanges.body.moderationAction,'AUTO_APPROVED');assert.equal((await req('/api/challenges/'+changesCreate.body.challenge.id)).status,200);pass('edited mission is immediately re-reviewed and published');
const rejectedCreate=await req('/api/challenges',{...challengeInput,title:'불법 해킹 대행자를 구합니다',description:'타인 계정에 무단 침입하는 불법 해킹을 수행하고 결과를 제출해주세요.'},b.cookie,'POST',env,{'Idempotency-Key':'moderation-reject-001'});assert.equal(rejectedCreate.status,201);assert.equal(rejectedCreate.body.moderationAction,'AUTO_REJECTED');assert.equal(rejectedCreate.body.challenge.status,'DRAFT');assert.ok(rejectedCreate.body.moderationReasons.some(x=>x.prohibited));assert.equal((await req('/api/challenges/'+rejectedCreate.body.challenge.id)).status,404);pass('prohibited mission is automatically rejected and retained privately');
assert.equal((await req('/api/challenges/'+rejectedCreate.body.challenge.id+'/moderation/appeals',{reason:'연구 목적의 합법적 보안 점검 요청이었으며 계약서와 권한 증빙을 제출할 수 있습니다.'},b.cookie)).status,201);pass('owner can appeal an automatic rejection');
...
const moderationOverview=await req('/api/admin/overview',undefined,b.cookie,'GET',adminEnv);assert.equal(moderationOverview.status,200);assert.ok(moderationOverview.body.overview.moderationStats.auto_approved>=1);assert.equal(moderationOverview.body.overview.moderationAppeals.length,1);pass('administrator sees automatic moderation statistics and appeals only');
...
const lifecycleEnv={...env,APP_ENV:'test',LOCAL_MONEY_SIMULATION:'true',PAYOUT_WEBHOOK_SECRET:'local-fixture-only'};
